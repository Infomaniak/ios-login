    ...
    public func getOrCreateCurrentDevice() async throws -> UserDevice {
        ...
        if FileManager.default.fileExists(atPath: deviceIdFileURL.path),
           ...
           let deviceIdString = String(data: deviceIdData, encoding: .utf8) {
            return UserDevice(id: deviceIdString)
        } else {
            ...
            return UserDevice(id: userDeviceUUID)
        }
    }

    public func attachCurrentDevice(to token: ApiToken) async throws {
        let currentDevice = try await getOrCreateCurrentDevice()
        try await attachDevice(currentDevice, to: token)
    }

    func attachDevice(_ device: UserDevice, to token: ApiToken) async throws {
        // TODO: API call
    }
}
